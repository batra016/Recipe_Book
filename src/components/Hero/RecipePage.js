...
const RecipePage = () => {
  ...
  if (loading) return <p className="text-center mt-10 text-gray-500">Loading recipe...</p>;
  ...
  return (
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="mb-12">
        <h2 className="text-2xl font-semibold mb-6 text-green-500">👩‍🍳 Cooking Instructions</h2>
        {steps.length > 0 ? (
          <ol className="space-y-6 list-decimal list-inside text-gray-800 text-[15px]">
            {steps.map((step) => (
              <li key={step.number} className="bg-gray-50 p-4 rounded-md shadow-sm border border-gray-200">
                <p className="mb-2">{step.step}</p>

                {step.ingredients?.length > 0 && (
                  <div className="flex flex-wrap gap-3 mt-2">
                    {step.ingredients.map((ing, idx) => (
                      <div key={idx} className="flex items-center gap-2 text-sm bg-green-100 text-green-800 px-2 py-1 rounded-full">
                        <img
                ...
                {step.equipment?.length > 0 && (
                  <div className="flex flex-wrap gap-3 mt-2">
                    {step.equipment.map((eq, idx) => (
                      <div key={idx} className="flex items-center gap-2 text-sm bg-gray-100 text-gray-700 px-2 py-1 rounded-full">
                        <img
                          src={eq.image}
                          alt={eq.name}
                          className="w-5 h-5 rounded-full"
      ...
      <div>
        <h2 className="text-2xl font-semibold mb-6 text-green-500">🧂 Ingredients</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {recipe.extendedIngredients?.map((ing, index) => (
            <div key={index} className="flex items-center gap-3 bg-white border border-gray-200 rounded-xl px-4 py-2 shadow-sm hover:shadow-md transition">
              <img
                src={`https://spoonacular.com/cdn/ingredients_100x100/${ing.image}`}
                alt={ing.name}
